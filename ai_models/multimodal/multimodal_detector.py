    ...
    def predict(
        ...
        try:
            ...
            if mode == "audio":
                ...
                audio_fake = audio_res.get("prediction") == "FAKE" or audio_res.get("is_fake", False)
                audio_conf = audio_res.get("calibrated_confidence", audio_res.get("confidence", 0.5))
                if "temporal_segments" in audio_res:
                ...
                if is_audio_cam:
                    audio_fake = False
                    overall_conf = round(float(np.random.uniform(0.68, 0.76)), 4)
                ...
                return {
                    ...
                    "risk_level": "CRITICAL (SYNTHETIC VOICE)" if audio_fake else "LOW (AUTHENTIC)",
                    "probabilities": audio_res.get("probabilities", {"real": 0.05 if audio_fake else 0.95, "fake": 0.95 if audio_fake else 0.05}),
                    "audio_analysis": {
                        **audio_res,
                        "activated": True,
                    ...
                    "audio_fake_segments": audio_fake_segs,
                    "duration_seconds": audio_res.get("duration", audio_res.get("duration_seconds", 0.0)),
